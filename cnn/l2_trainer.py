...
class Trainer:
		...
		for i, data in enumerate(self.dataset.train_loader, 0):
			for i in range(10000):
				...
				if(math.isnan(next_step_loss.item()) or next_step_loss.item() > 1e10 or next_step_loss.item() == 0):
					self.logger.error('next step loss nan')
				...
				if epoch >= 0 and (i + 1) % args.log_freq == 0:    # print every 2000 mini-batches
					...
					self.logger.info('[%d, %5d] loss: %8.2e/%7.1e, next step loss: %8.2e, beta L2: %7.1e, alpha[-1]: %s' %
						  (epoch + 1, i + 1, losses.mean(), model_grad_norms / args.log_freq, next_step_losses.mean(), beta_grad_norms / args.log_freq, alpha_out))
					model_grad_norms, beta_grad_norms = 0., 0.
		return True
	...
	def _train_epoch_fix_beta(self, epoch, lr, optimizee):
		...
		for i, data in enumerate(self.dataset.train_loader, 0):
			...
			loss.backward()
			model_grad_norms += nn.utils.clip_grad_norm_(optimizee.model.parameters(), args.model_grad_norm)
			_, = optimizee.optimizer.step(virtual=False, do_update=True)
			losses += loss.item()
			...
			if (i + 1) % max(args.log_freq, 50) == 0:    # print every 2000 mini-batches
				...
				self.logger.info('[%d, %5d] loss: %.3f/%.3f, alpha[-1]: %s' %
					  (epoch + 1, i + 1, losses / log_freq, model_grad_norms / log_freq, alpha_out))  
