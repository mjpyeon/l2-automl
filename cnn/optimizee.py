...
from auto_optimizer_nn import AutoOptimizer
...
class Optimizee:
		...
		def _iter_child(module):
			for child in module.children():
				if len(child._parameters) > 0:
					...
					_iter_child(child)
		_iter_child(self.symbolic_model)
		# when using nn as optmizer, detach the lstm hidden states as well
	...
	def beta_step(self, meta_update_loss):
		...
		for b in self.optimizer.optim_parameters():
			if b.grad is None:
				b.grad = torch.zeros_like(b)

		# check if grad is nan
		for beta in self.optimizer.optim_parameters():
		...
		# grad normalization
		beta_grad_norms = nn.utils.clip_grad_norm_(self.optimizer.optim_parameters(), args.beta_grad_norm)
