...
class PPO(object):
	...
	def __init__(self):
		...
		self.input_codes = tf.placeholder(tf.int32, [None, 78], 'input_codes')
		self.batch_size = tf.placeholder(tf.int32, [])
		self.sample_op = new_policy.sample_sequence(self.input_codes, self.batch_size)
	...
	def update(self, a, r, step, input_codes):
		...
		if METHOD['name'] == 'clip':
			 ...
			 [self.sess.run(self.atrain_op, {self.tfa: a, self.tfadv: adv, self.batch_size: batch_size, self.target_gather: a_gather_, self.input_codes:input_codes}) for _ in range(A_UPDATE_STEPS)]
	...
	def choose_sequence(self, input_codes):
		return self.sess.run(self.sample_op, {self.input_codes:input_codes, self.batch_size:len(input_codes)})
